...
library ReserveConfiguration {
    ...
    /// @dev Get decimals for memory data
    function getDecimalsMemory(DataTypes.ReserveConfigurationMap memory self) internal pure returns (uint256) {
        return (self.data & ~DECIMALS_MASK) >> RESERVE_DECIMALS_START_BIT_POSITION;
    }
    ...
    /// @dev Get reserve factor for memory data
    function getReserveFactorMemory(DataTypes.ReserveConfigurationMap memory self) internal pure returns (uint256) {
        return
            (self.data & ~RESERVE_FACTOR_MASK) >>
            RESERVE_FACTOR_START_BIT_POSITION;
    }
